...
x = dl.get_train_data_batch(2048, 0)
z = model.encode(x)
...
def get_random_z_batch(batch_size):
    x_batch = dl.get_test_data_batch(batch_size, norm=True)
...
def evaluate_losses(n_iter=100, batch_size=128):
    ...
    num_batches = dl.get_num_batches('test', batch_size)
    for i in range(n_iter):
        batch_id = np.random.randint(0, num_batches)
        x_batch = dl.get_test_data_batch(batch_size, batch_index=batch_id, norm=True)
...
if len(tasks.intersection({'recons', 'x_recons', 'z_recons', 'gif_recons'})) > 0:
    batch_size = dl.num_test_examples
...
if 'walks' in tasks:
    ...
    for i in range(5):
        ...
        x_batch = dl.get_test_data_batch(1024, 0, norm=True)
...
if 'zdist' in tasks:
    ...
    x_batch = dl.get_test_data_batch(z_dist_batch_size, 0, shuffle=True, norm=True)
    z_ = model.encode(x_batch)
